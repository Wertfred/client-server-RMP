...
Функция в sequlize позволяет записывать даннные через такой метод в get запросе:
...
Способ вывода картинки?
1) req.params = {id: '123',orderId: '456'}
2) ?page='1'&limit = '10'&search='John'}
3) req.body { 'name':'Ivan','rating':4}



# Модуль 4. Разработка клиентского приложения с помощью React

React приложение это часть Node.JS и для своей работы он использует отдельный порт(3000).
Разработка ведётся по принципу SPA(single paige application)
На одной странице содержиться статичный navbar с двумя ссылками на другие страницы(компоненты): товары, клиенты и заказы.
При нажатии на любую гиперссылку будет рендериться(отрисовываться) в нижнем контентном блоке соответствующая информация(блок с продуктами либо таблица с клиентами).
Приложение React состоят из компонентов. Компонент - это элемент пользовательского интерфейса(UI) со своей логикой и внешним видом, компонент может быть как небольшим, как кнопка, так и большим. Простыми словами, компоненты React - функция JS, которая возвращает разметку(JSX)
Жизненый цикл компонента делиться на 3 этапа:
1 этап. Монтирование.
Первое отображение на странице сразу после создания.
2 этап. Обновление.
Повторное отображение ранее выведенного компонента. Производится после изменения какого-либо из значений, относящимся к "реактивным" данным(пропсов, состояний и т.д.). Выполняется быстрее, чем мантирование
3 Этап. Размантирование.
Удаление компонента со страницы. Производится при программном удалении компонента, переходе на другой экран в процессе навигации, при переходе на другю веб страницу или закрытие браузера.

Это помогают реализовать hook-и. Они представляют собой обычные функции JS.

Prop - это значение, передоваемое текущему компоненту его родителем. Может быть любым и принадлежать любому типу(функциональному или объектному). Компонент может принимать произвольное кол-во prop-сов, в том числе ниодного.

useState импортируется из react и объявить переменную состояния внутри нашего компонента.

## Маршрутизация
Маршрутизацию компонентов можно настроить с помощью react Router.
BrowserRoute - используется для клиентской навигации, если используется браузер и есть backEnd.
HashRouter - для статических хостингов(github pages).
